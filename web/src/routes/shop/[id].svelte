...
<script lang="ts">
  ...
  async function redeem(ticketTokenId: ethers.BigNumber) {
    console.log(ticketTokenId);
    console.log(highlightTokenId);
    ...
    <div class="w-full border border-gray-300 rounded-md">
      ...
      {#if tickets.length === 0}
        <p class="p-4 text-sm text-gray-500">
          Sorry, you have no redeemible tickets. You can purchase ticket stubs in our marketplace or get your own ticket
          stub from attending a game.
        </p>
      {:else}
        {#each tickets as t}
